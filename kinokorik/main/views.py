...
from .models import AdvUser, SubCategory, Bb, AdditionalImage, Comment
from .forms import (
	UserRegisterForm,
    ChangeUserInfoForm,
    SearchForm,
...
def profile_bb_delete(request, pk):
    ...
    if request.method == 'POST':
	    bb.delete()
	    messages.add_message(request, messages.SUCCESS, 'Movie Removed')
	    return redirect('main:profile')
    else:
    	context = {'bb': bb}
...
class ChangeUserInfoView(SuccessMessageMixin, LoginRequiredMixin, UpdateView):
	"""docstring for ChangeUserInfoView"""
	model = AdvUser
	template_name = "main/change_user_info.html"#path to the template file
	form_class = ChangeUserInfoForm
	success_url = reverse_lazy('main:profile')
	send_messages = 'Personal data changed successfully'
 	
 	#we will extract the user key and save it in the user id attribute.
	def dispatch(self, request, *args, **kwargs):
		self.user_id = request.user.pk
		return super().dispatch(request, *args, **kwargs)

	#Retrieving a Recoverable Record
...
def by_category(request, pk):
    ...
    page = paginator.get_page(page_num)
    context = {'category': category, 'page': page, 'bbs': page.object_list, 'form': form}
...
def detail(request, category_pk, pk):
	...
	if request.method == 'POST':
		...
		if c_form.is_valid():
			...
			form = c_form
			messages.add_message(request, messages.WARNING, 'Comment doas not add')
